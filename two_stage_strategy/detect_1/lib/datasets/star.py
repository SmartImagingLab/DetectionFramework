...
class star(imdb):
    ...
    def _load_star_txt(self,indx):         ##  change from _load_pascal_annotation(self,indx)
        ...
        for ix in range(num_objs):      # if objs_all[ix][0] == 100:   # tip:The first is class
            # 依据flux值得不同生成不同的真实框！140以上的框padding设置为32
            padding = 10
            if objs_all[ix][3] < 100:
                padding = 5 + (objs_all[ix][3] // 20)   # 是否 * 2
            x1 = objs_all[ix][1] - padding
            x2 = objs_all[ix][1] + padding+1
            y1 = objs_all[ix][2] - padding
            y2 = objs_all[ix][2] + padding+1
            x1 = max(float(x1), 0)
            x2 = min(float(x2), cfg.TRAIN.TRIM_HEIGHT)           # x2 = min(float(x2), 256)
            y1 = max(float(y1), 0)
            y2 = min(float(y2), cfg.TRAIN.TRIM_HEIGHT)
            ishards[ix] = 0
