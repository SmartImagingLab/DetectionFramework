...
def vis_detections_old(im, class_name, dets, thresh=0.8):
    ...
    for i in range(dets.shape[0]):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])   # np.round(x):方法返回浮点数x的四舍五入值
        score = dets[i, -1]
        if score > thresh:
            if class_name == "EP":    # 修改
...
def vis_detections(im, dets, scale):
    """Visual debugging of detections.
    第一个参数：img是原图
    第二个参数：（x，y）是矩阵的左上点坐标
    第三个参数：（x+w，y+h）是矩阵的右下点坐标
    ...
    for i in range(dets.shape[0]):
        ...
        x2 = min(int(bbox[0:1]+bbox[2:3]/2), 512-1)
        y2 = min(int(bbox[1:2]+bbox[3:]/2), 512-1)
