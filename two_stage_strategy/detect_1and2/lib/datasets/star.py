...
class star(imdb):
    def __init__(self, image_set, data_path):
        imdb.__init__(self,image_set +'data300-300-2')
        self._image_set = image_set
        self._data_path = data_path
        self._devkit_path = ' /home/lab30202/lwb/EP/single_detect/single_telescope/fpn_star_single_torch_1_2/data'   # path
        ...
        self.fitsfilelist=glob.glob(self._data_path+'/'+self._image_set +'data300-300-2/'+'*.fit*')          # data
        self.positionfilelist=glob.glob(self._data_path+'/'+self._image_set +'data300-300-2/'+'*.list')      # label
    ...
    def _load_image_set_index(self):
        ...
        image_index = glob.glob(image_set_path+'/'+self._image_set +'data300-300-2/'+'*.fit*')
        image_index.sort()
    ...
    def _load_star_txt(self,indx):         ##  change from _load_pascal_annotation(self,indx)
        ...
        starposfilenametmp = self._data_path+'/'+self._image_set +'data300-300-2/'+ base_name+ '.list'
        ...
        if -1 in array_from_txt:
            ...
            return {}
        objs_all_list = []
        if array_from_txt.size>3:
            for step, value_single_row in enumerate(array_from_txt):
                if 0 < array_from_txt[step, 1] < 512 and 0 < array_from_txt[step, 2] < 512:  ##  remove nagative coordinate(存在无目标的图像-1，-1)
                    objs_all_list.append(value_single_row)
        else:
            if 0 < array_from_txt[1] < 512 and 0 < array_from_txt[2] < 512:  ##  remove nagative coordinate(存在无目标的图像-1，-1)
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self._devkit_path, 'results', 'EP_dectect', 'Main')
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        ovthresh overlap的阈值
        '''
        imagesetfile_path =  self._data_path+'/'+self._image_set +'data300-300-2/'+'*.fit*'
...
if __name__ == '__main__':
    d = star('train', '/home/lab30202/lwb/EP/single_detect/single_telescope')
    res = d.roidb
