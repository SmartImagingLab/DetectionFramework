    ...
    if len(det):  # 若是已经检测到目标
        ...
        while n:
            ...
            data = resize(img_fits[max(round(y1[n] / 8), 0): min(round(y2[n] / 8) + 1, 512),
                          max(round(x1[n] / 8), 0):min(round(x2[n] / 8) + 1, 512)], (object_size, object_size),
                          preserve_range=True)
...
if __name__ == '__main__':
    ...
    CUDA_VISIBLE_DEVICES = input('Choose which GPU to use:')
    os.environ["CUDA_VISIBLE_DEVICES"] = CUDA_VISIBLE_DEVICES
    print("GPU {} will be used\n".format(CUDA_VISIBLE_DEVICES))

    args = framework_config.parse_args()
    ...
    # 初始化指标:23个区间！
    flux_interval1 = np.append(np.linspace(0.1, 1.0, 10), np.linspace(1.25, 3, 8))  # 0.1, 0.25, 5, 50
    flux_interval2 = np.append(np.linspace(32, 90, 3), np.linspace(145, 200, 2))  # 0.1, 0.25, 5, 50
    flux_interval = np.append(flux_interval1, flux_interval2)  # 0.1, 0.25, 5, 50
    ...
    while (num_images > 0):  # num_images >=0  ？
        ...
        for i, contour in enumerate(contours):
            ...
            if w != 0 and h != 0:
                if x <= 0 or y <= 0:
                    ...
                    xy_point.append((x, y, x + w, y + h))  # 0 为 极亮源的标记！
        # 若是检测到极亮源，则将其从mask掉
        if xy_point:
            box_np_1 = np.array(xy_point)  # 按照列进行拼接！也可以是box_list
            # 搜寻亮源所在的位置
            xy_point_list, flux_list = get_xy_flux(box_np_1, img1, flag_0)
            # 统计真源数量
        ...
        if box_list:
            ...
            # 搜寻亮源所在的位置:中心点-宽和高
            point_list, box_list_flux = get_xy_flux(box_np_2, img1, flag_1)
        ...
        if box_list2:
            ...
            point_list_2, box_list_flux_2 = get_xy_flux(box_np_3, img2, flag_2)
        ...
        if args.RF_train:
            ...
            # 综合评价检测结果！
            true_num, recall_internal_num, precision_num, flux_internal_num, err_num = cal_indicators_old(
                result_save_path,
                all_box_np,
                flux_interval)
        ...
        all_targets = np.sum([3176.0, 320.0, 80.0, 70.0, 53.0, 256.0])
        if precision_all:
            precision_list.append(all_ture_num/precision_all)
            recall_list.append(np.sum(all_recall[-6:])/all_targets)
        # if num_images%10 == 0:
        #     print(all_ture_num)
        #     print(all_recall)
    ...
    # print("precision_list:", precision_list.sort())
    # print("recall_list:", recall_list)
    # 生成json文件
    data = {'precision_list':  precision_list, 'recall_list': recall_list}
    # 保存为json文件
    save_json("./precision_and_recall_list_3mcrab.json", data)

    # 生成json文件
    data1 = {'precision': precision, 'recall': recall.tolist(), "all_flux_internal_num": all_flux_internal_num.tolist()}
    # 保存为json文件
    save_json("./mul_step_json.json", data1)
